...
export class AirQualityStorage {
    ...
    async init() {
        try {
            ...
            if (this.isEnabled) {
                ...
                console.warn('⚠️ Storage service disabled - database connection failed');
            }
        } catch (error) {
    ...
    async storeStationData(stations) {
        ...
        const now = Date.now();
        if (this.lastStorageTime && (now - this.lastStorageTime) < this.storageInterval) {
        ...
        try {
            ...
            for (const station of stations) {
                ...
                if (!stationData.station_uid || !stationData.latitude || !stationData.longitude) {
                    console.warn('⚠️ Skipping station with missing essential data:', station);
                    continue;
                }
