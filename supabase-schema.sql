...
CREATE TABLE IF NOT EXISTS openweather_readings (
    id BIGSERIAL PRIMARY KEY,
...
-- Create indexes for OpenWeather readings
CREATE INDEX IF NOT EXISTS idx_openweather_location_time
ON openweather_readings(lat, lon, timestamp DESC);

CREATE INDEX IF NOT EXISTS idx_openweather_timestamp
ON openweather_readings(timestamp DESC);
