...
export function calculateThaiAQHI(pm25, o3, no2) {
  ...
  const pm25Component = pm25 ? 100 * (Math.exp(THAI_COEFFICIENTS.beta.pm25 * pm25) - 1) : 0;
  const o3Component = o3 ? (Math.exp(THAI_COEFFICIENTS.beta.o3 * o3) - 1) : 0;
  const no2Component = no2 ? (Math.exp(THAI_COEFFICIENTS.beta.no2 * no2) - 1) : 0;

  // Thai AQHI formula
  const aqhi = (10 / THAI_COEFFICIENTS.c) * (pm25Component + o3Component + no2Component);

  const finalAQHI = Math.max(0, Math.round(aqhi));

  console.log(`🎯 Thai AQHI components: PM2.5=${pm25Component.toFixed(4)}, O3=${o3Component.toFixed(4)}, NO2=${no2Component.toFixed(4)}`);
