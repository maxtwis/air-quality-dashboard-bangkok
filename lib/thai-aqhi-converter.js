...
const EPA_AQI_BREAKPOINTS = {
  ...
  o3_8hr: [
    [0, 50, 0.0, 0.054], // Good (ppm)
    [51, 100, 0.055, 0.07], // Moderate (ppm)
    [101, 150, 0.071, 0.085], // Unhealthy for Sensitive (ppm)
    [151, 200, 0.086, 0.105], // Unhealthy (ppm)
    [201, 300, 0.106, 0.2], // Very Unhealthy (ppm)
  ],
...
export function convertStationDataForThaiAQHI(stationData) {
  ...
  pollutants.forEach((pollutant) => {
    ...
    if (typeof aqiValue === "number") {
      ...
      if (thaiUnit !== null) {
        ...
        conversionLog.push(
          `${pollutant.toUpperCase()}: ${aqiValue} AQI → ${thaiUnit} ${unit}`,
        );
      }
    }
  });
  ...
  if (conversionLog.length > 0) {
    console.log(
      `🇹🇭 [THAI AQHI] Converting ${stationName}: ${conversionLog.join(", ")}`,
    );
  }
 ...
 */
export function calculateThaiAQHI(pm25, o3, no2, pm10 = null) {
  console.log(
    `🇹🇭 Calculating Thai AQHI: PM2.5=${pm25}μg/m³, PM10=${pm10}μg/m³, O3=${o3}ppb, NO2=${no2}ppb`,
  );
  ...
  const THAI_COEFFICIENTS = {
    ...
    beta: {
      pm25: 0.0022, // Per μg/m³
      pm10: 0.0009, // Per μg/m³
      o3: 0.001, // Per ppb
      no2: 0.003, // Per ppb
    },
  };
  ...
  const perErPM25 = pm25
    ? 100 * (Math.exp(THAI_COEFFICIENTS.beta.pm25 * pm25) - 1)
    : 0;
  const perErPM10 = pm10
    ? 100 * (Math.exp(THAI_COEFFICIENTS.beta.pm10 * pm10) - 1)
    : 0;
  const perErO3 = o3 ? 100 * (Math.exp(THAI_COEFFICIENTS.beta.o3 * o3) - 1) : 0;
  const perErNO2 = no2
    ? 100 * (Math.exp(THAI_COEFFICIENTS.beta.no2 * no2) - 1)
    : 0;
  ...
  console.log(
    `🎯 Thai AQHI %ER: PM2.5=${perErPM25.toFixed(2)}%, PM10=${perErPM10.toFixed(2)}%, O3=${perErO3.toFixed(2)}%, NO2=${perErNO2.toFixed(2)}%, Total=${totalPerER.toFixed(2)}%`,
  );
