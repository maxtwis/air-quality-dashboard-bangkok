...
if (
  ...
  console.warn(
    "⚠️ Supabase not configured. Please update supabase.js with your credentials or set window.SUPABASE_URL and window.SUPABASE_ANON_KEY",
  );
}
...
export class AirQualityDB {
  // Add supabase reference for upsert operations
  static get supabase() {
    return supabase;
  }
  ...
  /**
   * Get current 3-hour moving averages for all stations (WAQI+Google merged)
   */
  static async get3HourAverages(stationId = null) {
    let query = supabase.from("combined_3h_averages").select("*");
  ...
  /**
   * Get historical data for station chart (24 hours or 7 days)
   */
  static async getStationHistory(stationId, hours = 24) {
    ...
    if (isDevelopment) {
      // Use proxy endpoint
      try {
        const response = await fetch(`/api/supabase/station-history/${stationId}?hours=${hours}`);
        if (!response.ok) {
        ...
        console.error("Error fetching station history via proxy:", error);
        throw error;
      }
    } else {
      ...
      const { data, error } = await supabase
        .from("waqi_data")
        .select("timestamp, pm25, pm10, o3, no2, so2, co, aqi")
        .eq("station_uid", stationId)
        .gte(
          "timestamp",
          new Date(Date.now() - hours * 60 * 60 * 1000).toISOString(),
