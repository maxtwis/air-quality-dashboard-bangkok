...
const SUPABASE_URL =
  process.env.SUPABASE_URL || "https://plrjbynejtbuawxijejf.supabase.co";
...
async function testInsert() {
  ...
  try {
    ...
    await supabase.from("stations").delete().eq("uid", 99999);
...
async function testQuery() {
  ...
  try {
    ...
    const { data: aqhiResult, error: aqhiError } = await supabase.rpc(
      "calculate_aqhi",
      {
        pm25_val: 50,
        no2_val: 30,
    ...
    console.log(
      `✅ Found ${averages?.length || 0} stations with 3-hour averages`,
    );
...
async function getStats() {
  ...
  try {
    ...
    console.log("├─────────────────────────────┤");
    console.log(
      `│ Total Readings:    ${String(readingsCount || 0).padStart(8)} │`,
    );
    console.log(
      `│ Total Stations:    ${String(stationsCount || 0).padStart(8)} │`,
    );
    console.log(
      `│ Active Stations:   ${String(activeStationsCount || 0).padStart(8)} │`,
    );
    console.log("└─────────────────────────────┘");
