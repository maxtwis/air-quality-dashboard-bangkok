-- FRESH SCHEMA FOR BANGKOK AIR QUALITY DASHBOARD
-- Separate tables for WAQI and Google supplements
...
CREATE TABLE IF NOT EXISTS public.waqi_data (
...
CREATE OR REPLACE VIEW latest_waqi_readings AS
...
CREATE OR REPLACE VIEW latest_combined_readings AS
SELECT
    w.station_uid,
...
CREATE OR REPLACE VIEW waqi_3h_averages AS
SELECT
    station_uid,
    ...
    AVG(so2) as avg_so2,
    AVG(co) as avg_co,
    AVG(o3) as avg_o3,    -- Rarely populated
    AVG(no2) as avg_no2,  -- Rarely populated
...
CREATE OR REPLACE VIEW combined_3h_averages AS
SELECT
    w.station_uid,
...
AS $$
BEGIN
    -- Delete WAQI data older than 7 days
    DELETE FROM public.waqi_data
    WHERE timestamp < NOW() - INTERVAL '7 days';

    -- Delete Google supplements older than 7 days
