...
class PM25OnlySupabaseAQHI {
    ...
    initSupabase() {
        try {
            ...
            console.warn('⚠️ PM2.5-only AQHI calculator failed to initialize:', error);
            this.supabase = null;
        }
    }
    ...
    async getBatch3HourAverages(stationIds) {
        ...
        try {
            ...
            if (!data || data.length === 0) {
                console.log(`ℹ️ No stored PM2.5 data for ${stationIds.length} stations`);
                return {};
            }
            ...
            console.log(`📊 PM2.5-only batch processed ${Object.keys(stationAverages).length}/${stationIds.length} stations with data`);
    ...
    async calculatePM25OnlyAQHI(station) {
        ...
        try {
            ...
            if (averages && averages.pm25) {
                ...
                console.log(`🔄 PM2.5-only AQHI from 3h avg for ${stationId}: ${aqhiValue} (${averages.readingCount} readings)`);
                const aqhiLevel = getAQHILevel(aqhiValue);
                return {
                    value: aqhiValue,
                    level: aqhiLevel
            ...
            console.warn(`⚠️ Error calculating PM2.5-only AQHI for ${stationId}:`, error);
        }
    ...
    async enhanceStationsWithPM25OnlyAQHI(stations) {
        console.log(`🔄 Processing ${stations.length} stations with PM2.5-only AQHI...`);
        ...
        const enhancedStations = await Promise.all(
            stations.map(async (station) => {
                ...
                const aqhiData = {
                    ...
                    calculationMethod: averages && averages.pm25 ? 'stored_3h_avg' : 'current',
                    readingCount: averages ? averages.readingCount : 0,
                    timeSpanHours: averages ? Math.min(3, averages.readingCount * 0.17) : 0, // Estimate hours based on readings
                    dataQuality: averages && averages.readingCount >= 15 ? 'excellent' :
                                averages && averages.readingCount >= 10 ? 'good' :
                                averages && averages.readingCount >= 5 ? 'fair' :
                                averages && averages.readingCount > 0 ? 'limited' : 'estimated'
                };
        ...
        console.log(`✅ Enhanced ${enhancedStations.length} stations with PM2.5-only AQHI (${storedCount} from stored data) in ${duration}ms`);
